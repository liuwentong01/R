...
答：`steer` 是默认模式，它会在当前回合执行完工具调用、下一次模型调用之前，把中途到达的新消息注入当前运行，所以不会另起一轮；`followup` 更保守，等当前回合结束后，把新消息作为后续 turn 逐条处理；`collect` 则在安静窗口后，把排队消息合并成单个 followup 再跑。此外还有 `interrupt`，会直接中止当前运行去处理最新消息。面试时要强调：它们的区别不是“排不排队”，而是“如何处理中途新消息”。
