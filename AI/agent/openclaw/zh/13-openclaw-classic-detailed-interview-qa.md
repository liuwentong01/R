...
`steer` 是系统默认模式。它会在当前回合执行完工具调用、下一次模型调用之前，把中途到达的新消息注入当前运行，相当于在不另起一轮的前提下动态补充指令；如果当前运行无法接受 steering，就等它结束后再处理。`collect` 则偏向对话体验，会在安静窗口过后，把等待中的多条消息合并成一次 followup 输入，减少“你发三句、我回三次”的碎片感。
