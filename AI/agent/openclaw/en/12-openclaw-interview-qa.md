...
A: `collect` is the default mode, which merges queued messages and then runs the next turn; `followup` waits until the current turn finishes, then takes new messages as the input for the next turn; `steer` is more aggressive, trying to "insert" new messages into the current execution at tool-call boundaries. In an interview, emphasize: the difference is not "whether to queue," but "how to handle new messages that arrive mid-run."
