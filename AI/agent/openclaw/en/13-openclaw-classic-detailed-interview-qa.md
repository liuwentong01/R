...
`steer` is the system's default mode. After the current turn finishes executing its tool calls and before the next model call, it injects mid-run messages into the active run—essentially adding instructions dynamically without starting a second turn; if the active run cannot accept steering, it waits for that run to finish first. `collect` leans toward conversation experience: after a quiet window, it merges the multiple waiting messages into a single followup input, reducing the fragmented feeling of "you send three sentences, I reply three times."
