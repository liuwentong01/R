...
如果用 极简版讲这篇，最重要的是先分清 `Tool` 和 `Skill`。`Tool` 是原子操作，比如读文件、发请求、执行命令；`Skill` 则是把一类专业能力打包起来，它通常包含指令、工具集和上下文知识，等于给 Agent 装上一块可复用的“能力模块”。没有 Skills 的时候，所有知识都要塞进一个巨大 prompt 里，结果就是提示词越来越长、能力互相干扰，而且难以复用。

这篇文章把 Skills 分成三种实现方式。第一种是 `Prompt-as-Skill`，也就是最常见、最实用的方式，用结构化 Markdown 或规则文件定义一类能力；第二种是 `Plugin-as-Skill`，把能力做成更强的插件或扩展；第三种是 `Agent-as-Skill`，直接把一个专精 Agent 当成可调用能力。无论是哪一种，背后的核心都是一样的：把“知识 + 策略 + 工具使用方式”做成可以独立加载、组合和演化的模块。

真正值得记住的还有两点。第一，Skill 不是越大越好，最好遵循单一职责、明确输入输出、支持渐进增强。第二，Skills 的价值不只是复用，更是在和上下文窗口做博弈，把本来必须常驻主提示词的能力，变成按需注入。换句话说，Skills 是现代 Agent 架构从“一个大脑包打天下”走向“模块化能力系统”的关键一步。
