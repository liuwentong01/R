...
如果用 极简版讲这篇，重点是：今天绝大多数 Agent，本质上都还是单 Agent，只是内部循环设计不同。最经典的模式是 `ReAct`，也就是“思考一下，调用一个工具，观察结果，再继续思考”，这套 `Think -> Act -> Observe` 几乎已经成了工业界默认范式，因为它简单、通用，而且足够适合大多数真实任务。

但不是所有任务都适合纯 `ReAct`。如果任务很长、步骤比较明确，就更适合 `Plan-and-Execute`，先把事情拆成计划，再按计划执行，这样成本更可控，也不容易在中途乱跑。如果任务失败率高、需要不断试错，就会引入 `Reflexion`，也就是让 Agent 在失败后总结原因，再带着“反思”进入下一轮。再往上，还有 `ReWOO` 和 `LATS` 这种更高级的研究型模式，一个强调先规划变量和依赖，减少实时观察；一个强调像树搜索一样探索多条可能路径。

真正有价值的结论不是记住这些名字，而是知道怎么选。工业系统里最常见的仍然是增强版 `ReAct`，复杂任务会叠加规划能力，代价高的任务会补反思机制。与此同时，单 Agent 真正难的地方不在论文名字，而在工程问题，比如什么时候停、上下文怎么裁剪、出错怎么恢复。单 Agent 做不好，多 Agent 只会把复杂度继续放大。
