...
如果用 极简版讲这篇，重点不是背框架名单，而是看清每个框架在回答什么问题。`Claude Code` 代表的是“极简 Agentic Loop + 本地工具 + 上下文工程”，非常适合 CLI 编码场景；`OpenAI Agents SDK` 更像生产级通用框架，重点在 `handoff`、`guardrails` 和 `tracing`；`LangGraph` 强在图状态机，适合复杂编排；`CrewAI` 强调角色分工和团队协作；`Cursor Agent` 是 IDE 深度集成路线；`OpenAI Codex` 则把 Agent 放进云端沙箱，走异步任务执行。

所以这篇最想说明的是：框架差异的本质，不在语法，而在架构哲学。有人追求极简循环，有人追求可编排性，有人追求角色协作，有人追求 IDE 体验，还有人追求云端隔离和任务化。选型时不能问“哪个最强”，而要问“我的问题更像哪一类”。比如做开发助手，Claude Code、Cursor 很自然；做生产环境的多 Agent 系统，Agents SDK 和 LangGraph 会更合适。

但差异之外也有共识。几乎所有主流框架都在往几个方向收敛：都要有稳定的工具调用、都要重视上下文管理、都在补可观测性和安全边界、都开始支持多 Agent 或多阶段协作。也就是说，框架表面上很多，底层正在形成一套越来越清晰的 Agent 基础设施共识。
