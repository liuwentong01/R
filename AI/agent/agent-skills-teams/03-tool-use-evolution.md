...
如果用 极简版讲这篇，可以把它理解成一句话：`Tool Use` 的演进，其实就是 Agent 和外部世界连接方式的演进。最早的做法非常原始，靠提示词告诉模型“你可以输出某种 JSON 来调用工具”，这就是第一代 `Prompt Hacking`。它能工作，但很脆弱，格式经常错，多工具调用难，安全性也差，本质上是在“骗模型按格式说话”。

第二代的关键转折是 `Function Calling`。从这一步开始，工具调用不再是提示词技巧，而是模型 API 的原生能力，参数结构、类型约束和多轮调用都稳定了很多，这也是 Agent 真正能进入工程实践的基础。再往后，行业发现光有函数调用还不够，因为很多工具不是一次性无状态的，而且不同平台各搞一套，生态会很碎，所以才有了第三代的 `Assistants / 有状态工具 / MCP`。`MCP` 最重要的意义，不只是多一个协议，而是让“工具和资源如何暴露给模型”开始标准化。

第四代则继续往前走，从 `Tool Use` 走向 `Agent Use`，也就是 Agent 不只是调用函数，还能调用另一个 Agent，这就是 `A2A` 这类协议出现的背景。所以这篇真正想传达的是：Agent 能力的上限，很大程度取决于工具协议设计。工具粒度要合适、描述要精准、输入输出契约要稳定，否则再强的模型也会因为工具层设计差而表现失真。
